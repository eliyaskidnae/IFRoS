...
class DR_3DOFDifferentialDrive(Localization):
    ...
    def Localize(self, xk_1, uk):  # motion model
        ...
        N_L = uk[0,0] # Number of Left wheel pulse
        N_R = uk[1,0] # Number Right  wheel pulse
        dl = (N_L*2*pi*self.wheelRadius)/  (self.robot.pulse_x_wheelTurns ) # Left wheel distance 
        dr = (N_R*2*pi*self.wheelRadius)/ (self.robot.pulse_x_wheelTurns )  # Right wheel distance 
        
        #find the 
        d = (dl + dr )/2 # point laying int the wheel axis and equidistance  to bot wheels     
        delata_teta = (dr-dl)/(self.wheelBase) # when th curve is approximate to a line
        teta_k = delata_teta + xk_1[2][0]
        
        xk = [[(xk_1[0][0] + d*cos(teta_k))] , [(xk_1[1][0]+d*sin(teta_k))] ,[teta_k]] # updating the current position   
        
        # xk = xk_1.oplus(np.array([[d*cos(teta_k)] ,d*sin(teta_k)]))
